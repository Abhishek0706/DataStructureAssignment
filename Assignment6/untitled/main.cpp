...
/**
 * @brief strict for a point in a graph
 */
struct Point
{
	char letter;
...
/**
 * @brief struct for a line in a graph
 */
struct Line
{
	Point beg, end;
	Line &operator=(Line const &b)
	{
		beg = b.beg;
		end = b.end;
		return *this;
...
/**
 * @brief method for finding the intersection
 * 
 */
...
{
	{
		...
		for (int k = 0; k < 2; ++k)
		{
			...
			if (!flag)
				cout << "Cannot find proper ending event for Line" << endl;
		}
	}
...
/**
 * @brief method for finding intersection of two lines
 * 
 */
	...
	while (!events.empty())
	{
		...
		if (t == seg_start)
		{
			...
			if (print)
			{
				...
				{
					cout << it->first.letter << "(" << it->first.x << "," << it->first.y << ")"
						 << " Line " << it->second << endl;
				}
			}
				...
				while ((dir ? ++iter : iter--) != (dir ? sweep.end() : sweep.begin()))
				{
					...
					if (lines[s].beg != lines[iter->second].beg)
					{
						...
						if (I.first)
						{
							...
							if (print)
								cout << "Intersection of " << s << " and " << iter->second << " at " << I.second.letter << "(" << I.second.x << "," << I.second.y << ")" << endl;
							intersections.push_back(I.second);
							intersect(s, iter->second, I.second, lines, sweep, events, print);
						}
					}
					else if (print)
						cout << "skipping lines with same starting point" << endl;
		...
		else if (t == seg_end)
		{
			...
			{
				if (print)
					cout << "test " << below->second << " with " << above->second << endl;
				if (lines[above->second].beg != lines[below->second].beg)
				{
					...
					if (I.first)
					{
						I.second.letter += (char)intersections.size();
						if (print)
							cout << "Intersection of " << below->second << " and " << above->second << " at " << I.second.letter << "(" << I.second.x << "," << I.second.y << ")" << endl;
						intersections.push_back(I.second);
...
/**
 * @brief method for finding a linear fit line with given cordinates of points
 */
...
int main(int argc, char *argv[])
{
	...
	io->setScale(1.5);
	scene->addItem(io);
	view->setWindowTitle("Coordinate-axis");
