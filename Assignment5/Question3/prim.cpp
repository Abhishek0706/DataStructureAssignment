...
void printArr(int arr[], int n,string *arr_node,int *arr_weight) 
{ 
	for (int i = 1; i < n; ++i) 
    {
		if(arr[i]<i)
...
void PrimMST(struct Graph* graph,string *arr_node,int *arr_weight) 
{ 
	...
	// print edges of MST 
	printArr(parent, V,arr_node,arr_weight); 
} 
...
{ 
    ...
    for (int i = 0; i < E; i++)
    {
        ...
        addEdge(graph, (int)(*(record+0))-65, (int)(*(record+2))-65, *(record+4)-48); 
    } 

	PrimMST(graph,arr_node,arr_weight); 
